...
clear;
close all;
datadirs = {'DWdc3_lcode_x1','DWdc3_lcode_x2','DWdc3_lcode_x4',...
    'DWdc3_lcode_x5','DWdc3_lcode_x666','DWdc3_lcode_x10'};
...
dephasing_xis = [1,7.5,15];
dephasing_search = '0x'; % 0x, max
force_waterfall = false;

% initialize some plotty
P1 = Plotty('plots_dir','DW/dephasing','save_format',save_format);
...
for ph = 1:length(dephasing_xis)
    ...
    for d = 1:length(datadirs)
        ...
        plot((-OPA.dephasing_line)*OPA.plasma_wavelength+OPA.simulation_window-OPA.dephasing_first,...
            linspace(OPA.waterfall_z(1)/545.39872233,OPA.waterfall_z(2)/545.39872233,length(OPA.dephasing_line)),...
            'LineWidth',2,'Parent',ax,'color','k')
        %         for xx = 1:length(bunches_x)
        %         scatter(bunches_x,bunches_y,'LineWidth',2,'Parent',ax,...
        ...
        switch datadir
            case 'DWdc3_lcode_x1'
                title(['density 100\% ','$\xi = ',num2str(dephasing_xi),'$ cm'],'Interpreter','latex')
            case 'DWdc3_lcode_x2'
                title(['density 50\% ','$\xi = ',num2str(dephasing_xi),'$ cm'],'Interpreter','latex')
            case 'DWdc3_lcode_x4'
                title(['density 25\% ','$\xi = ',num2str(dephasing_xi),'$ cm'],'Interpreter','latex')
            case 'DWdc3_lcode_x5'
                title(['density 20\% ','$\xi = ',num2str(dephasing_xi),'$ cm'],'Interpreter','latex')
            case 'DWdc3_lcode_x666'
                title(['density 15\% ','$\xi = ',num2str(dephasing_xi),'$ cm'],'Interpreter','latex')
            case 'DWdc3_lcode_x10'
                title(['density 10\% ','$\xi = ',num2str(dephasing_xi),'$ cm'],'Interpreter','latex')
        end
