...
sigzs = 50995.5; % sigma z as in input.f
sigr = 0.141654; % sigma r as in input.f
xis = xmax - 2.0;
% xiback = xmax - ;
xicg1 = -100.924; % center
nbn0 = 0.158734; 
...
% electron bunch
sigze = 1.27489; % sigma z as in input.f
sigre = 1.03266; % sigma r as in input.f
xice = 138.32;
nbne = -0.0498993;

electron = nbne*exp(-((XI-xice).^2)/sigze).*exp(-(R.^2)/sigre);
% electron(:,ind1e:nx1) = 0;
% electron(:,1:ind2e) = 0;
...
r0Lb_a = (r.').*LGe.*besseli0_access.';
r0Lb_a_wz = r0Lb_a;
rinfLb_a = (r.').*LGe.*besselk0_access.';
