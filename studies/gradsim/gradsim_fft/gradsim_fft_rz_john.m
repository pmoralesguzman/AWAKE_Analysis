...
% save directory
plots_dir = ['gradsim/fft/rz/',datadirs{1},''];
plot_name_suffix = [''];
...
trans_lims = 0.018*(1:9); %(1:nslices)/nslices*plasma_radius;
% trans_lims = ...
...
for d = 1:length(datadirs)
    ...
    %% here
    fvsrz = figure;
    trans_lims_m = (trans_lims*10)' - 0.09; % trans lims in mm
    prop_distance_m_mat = repmat(prop_distance_m,length(trans_lims),1);
    hold on
    
    sc_handle = imagesc(prop_distance_m_mat(1,:),trans_lims_m,...
        peak_freqs_plot);
    ...
    for rr = length(trans_ranges):-1:1
        ...
        for p = 0:dump_fft-2
            ...
            pave = plot(zave_plot(rr,2*(p+0.5):2*(p+0.5)+2), rave_plot(rr,2*(p+0.5):2*(p+0.5)+2),...
                'color',[1 1 1],'LineWidth',linewidths(rr,2*p+1)); %(rr,2*p+1)
            hold off %color_meanline(rr,:)
