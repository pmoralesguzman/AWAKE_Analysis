...
% save directory
plots_dir = ['gradsim/fft/r/','n',num2str(dump),''];
...
trans_lims_exp  = r_step*(1:n_r_step); % cm
...
P = Plotty('plasmaden',plasma_density,'plots_dir',plots_dir,'save_format',save_format,...
    'datadir',datadirs{1});
plot_name = ['fvsr',datadirs{1},'m',num2str(measurement_distance)];
...
xlimits = [-1.01*trans_lims_mm(end) 0];


hold on
scatter([flipud(-trans_lims_mm)],...
    [flipud(simpeak_freqs)],[dotsize],flipud(color_meanline),'filled');
% 
% scatter([flipud(-trans_lims_mm);trans_lims_mm],exppeak_freqs,...
%     [dotsize;dotsize],'x','LineWidth',1); % experiment
% plot(xlimits,freq_widerange*ones(1,2)/AFFT.plasmafreq_GHz,'--b','LineWidth',2);
yline(AFFT.plasmafreq_GHz,'--r','LineWidth',2);
yline(AFFT.plasmafreq_GHz*(0.8^0.5),'--b','LineWidth',2);
