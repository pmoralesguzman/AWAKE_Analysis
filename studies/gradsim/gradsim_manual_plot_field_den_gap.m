...
plots_dir = ['gradsim/','field_density_longprofile_all/'];
...
t1 = tiledlayout(tile_handle,4,1,'TileSpacing','compact');
t1.Layout.Tile = 1;
t2 = tiledlayout(tile_handle,4,1,'TileSpacing','compact');
t2.Layout.Tile = 2;
t3 = tiledlayout(tile_handle,4,1,'TileSpacing','compact');
t3.Layout.Tile = 3;
...
for d = 1:length(datadirs)
    ...
    ind_xi = (z_plot > xi_range(1)) & (z_plot < xi_range(2));
    density_plot = EDA.SCI(ind_r,ind_xi);
    r_plot = r_plot(ind_r);
    ...
    ind_opaque(density_plot > meanstd_density) = meanstd_density;
%     if d == 1
        max_indopaque = max(ind_opaque,[],'all');
%     end
    ind_opaque = max_opaqueness*ind_opaque/max_indopaque;
...
%     axlongprofile_exp(d).XTickLabel = [];
    ...
    ind_xi = (z_plot > xi_range(1)) & (z_plot < xi_range(2));
    density_plot_half = simtimeprofile(ind_r,ind_xi);
    r_plot = r_plot(ind_r);
    z_plot = z_plot(ind_xi);
