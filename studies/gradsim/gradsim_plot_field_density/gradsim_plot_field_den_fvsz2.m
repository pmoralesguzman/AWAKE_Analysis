...
% trans_ranges = [zeros(101,1),sigma_beam'];
...
for rr = 1:length(sigma_beam)

P = Plotty('datadir',datadir,'dataformat',dataformat,...
    'useAvg',useAvg,'dump_list',dump_list(rr),...
...
figure(fig_number);
P.plot_field_density('ylinepos',[-sigma_beam(rr),sigma_beam(rr)],'ylineflag',1,...
