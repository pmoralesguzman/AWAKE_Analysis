...
% save directory
plots_dir           = ['gradsim_paper/ch/chvsg/',''];
plot_name_suffix    = [''];
...
exp_normalization   = (1 - exp(-(sigma_normalization)^2/2))*initial_charge;

% initialize charges
charge_translim = zeros(length(trans_limit),length(grads_sim));
charge = zeros(length(trans_limit),length(grads_sim));
...
for d = 1:length(datadirs)
    ...
    for r = 1:length(trans_limit)
        ...
        charge(r,d) = charge_translim(r,d) ...
            + bunch_back_fraction*initial_charge;
        O.progress_dump('charge fraction r',r + (d-1)*length(trans_limit),length(trans_limit)*length(datadirs));
    end
    
end
sim_charge = charge/exp_normalization + 0.2914;
