...
trans_ranges = [0,0.018*(1:8)]' + [0.0 0.018]; %(0:0.01:0.14)' + [0.0 0.01]; % cm
...
for rr = 1:length(trans_ranges)
    ...
    r_position_tracks = ceil(rave(rr,:)/0.18); % look here
    ...
    for p = 1:dump_fft+1
        for rrr = min(r_position_tracks):max(r_position_tracks)
            ...
            ind_pks = (AFFT.locs(maxpk_loc) > (peak_freqs_t(rr) - 0.3860/2)) & (AFFT.locs(maxpk_loc) < (peak_freqs_t(rr) + 0.3860/2));
...
end % for length(trans_ranges) 

linewidths = 6*linewidths./max(linewidths,[],2);
linewidths_isnan = isnan(linewidths);
linewidths(linewidths_isnan) = 1e-10;
linewidths(~linewidths_isnan) = 4; %1e-10;


ind5 = (zave > 0.9) & (rave > 0.3);
ind_nan = linewidths < 1;
rave_plot = rave;
rave_plot(ind_nan & ~ind5) = nan;
zave_plot = zave;
zave_plot(ind_nan & ~ind5) = nan;
linewidths(linewidths_isnan) = 4;

disp('powerspectra calculation done')
...
for rr = length(trans_ranges):-1:1
    plot(zave(rr,:),rave(rr,:),'color',[0.7 0.7 0.7],'LineWidth',4);
    for p = 0:dump_fft-1
        ...
        OPT.progress_dump('plot lines',p,100)
    end
end

imagesc([100 101],[100 101],peak_freqs_t([1,9])');
hold off

fig1.CurrentAxes.FontUnits = 'centimeters';
fig1.CurrentAxes.FontSize = 0.4;
fig1.Units = 'centimeters';
fig1.Position = [1,1,8.6,8.6*3/4]*1.5;
...
c = colorbar(ax_colorbar);
c.Label.Interpreter = 'latex';
c.Label.String = '$f_{\mathrm{mod}}$ (GHz)';
yticks(0:0.18:1.5);

P.plot_name = 'trackcolor';
P.fig_handle = fig1;
P.save_plot();
