...
P = Plotty('datadir',datadir,'extradatadir',extradatadir,'dataformat',dataformat,...
    'useAvg',useAvg,'dump_list',dump_list,'dump',dump_list,...
...
P.dump = 0;
P.property = 'phasespace';
P.getdata(); P.trim_data();
r_plot = P.r';
angle_r = P.ndataOut;

mean_angle_r = mean((angle_r),2,'omitnan');
x = figure(4);
plot(r_plot,mean_angle_r,'k','LineWidth',2)
xlabel('r','Interpreter','latex')
ylabel('angle','Interpreter','latex')
title('total (positive + negative)')
P.fig_handle = x;
P.plot_name = '01';
P.save_plot();
